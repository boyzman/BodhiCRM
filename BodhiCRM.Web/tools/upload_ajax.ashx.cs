...
namespace BodhiCRM.Web.tools
{
    ...
    public class upload_ajax : IHttpHandler, IRequiresSessionState
    {
        ...
        {
            Model.SYS_CONFIG sysConfig = new BLL.SYS_CONFIG().loadConfig();
            ...
            if (!string.IsNullOrEmpty(_delfile) && _delfile.IndexOf("../") == -1 
                && _delfile.ToLower().StartsWith(sysConfig.webpath.ToLower() + sysConfig.filepath.ToLower()))
            {
        ...
        private void ManagerFile(HttpContext context)
        {
            ...
            //根目录路径，相对路径
            String rootPath = sysConfig.webpath + sysConfig.filepath + "/"; //站点目录+上传目录
            //根目录URL，可以指定绝对路径，比如 http://www.yoursite.com/attached/
            String rootUrl = sysConfig.webpath + sysConfig.filepath + "/";
            //图片扩展名
