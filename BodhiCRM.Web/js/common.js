...
$(function(){
    ...
    $(".rule-single-checkbox-sex").ruleSingleCheckbox_Param("女", "男");
	$(".rule-multi-checkbox").ruleMultiCheckbox();
	$(".rule-multi-radio").ruleMultiRadio();
	$(".rule-single-select").ruleSingleSelect();
	$(".rule-single-selectOnselected").ruleSingleSelectOnSelected();
...
$.fn.ruleSingleCheckbox_Param = function (s,t) {
    var singleCheckbox = function (parentObj) {
        //查找复选框
        var checkObj = parentObj.children('input:checkbox').eq(0);
        parentObj.children().hide();
...
//单选下拉框(带无刷新联动)
$.fn.ruleSingleSelectOnSelected = function () {
    var singleSelect = function (parentObj) {
        parentObj.addClass("single-select"); //添加样式
        parentObj.children().hide(); //隐藏内容
        var divObj = $('<div class="boxwrap"></div>').prependTo(parentObj); //前插入一个DIV
        ...
        selectObj.find("option").each(function (i) {
            ...
            liObj.click(function () {
                ...
                $.ajax({
                    type: "post",
                    url: "../tools/admin_ajax.ashx?action=get_sms_templete&time=" + Math.random(),
                    dataType: "html",
...
//根据生日获取年龄
function getAges(bdate) {
    var r = bdate.match(/^(\d{1,4})(-|\/)(\d{1,2})\2(\d{1,2})$/);
