...
namespace BodhiCRM.BLL
{
	...
	{
		private readonly BodhiCRM.DAL.M_VIPSET dal=new BodhiCRM.DAL.M_VIPSET();
		public M_VIPSET()
		{}
		#region  BasicMethod
		/// <summary>
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public BodhiCRM.Model.M_VIPSET GetModelByCache(decimal ID)
		{
			
			string CacheKey = "M_VIPSETModel-" + ID;
			...
			return (BodhiCRM.Model.M_VIPSET)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<BodhiCRM.Model.M_VIPSET> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<BodhiCRM.Model.M_VIPSET> DataTableToList(DataTable dt)
		{
			List<BodhiCRM.Model.M_VIPSET> modelList = new List<BodhiCRM.Model.M_VIPSET>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				BodhiCRM.Model.M_VIPSET model;
