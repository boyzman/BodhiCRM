...
namespace BodhiCRM.BLL
{
	...
	{
		private readonly BodhiCRM.DAL.VISIT_DETAIL dal=new BodhiCRM.DAL.VISIT_DETAIL();
		public VISIT_DETAIL()
		{}
		#region  BasicMethod
		/// <summary>
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public BodhiCRM.Model.VISIT_DETAIL GetModelByCache(decimal ID)
		{
			
			string CacheKey = "VISIT_DETAILModel-" + ID;
			...
			return (BodhiCRM.Model.VISIT_DETAIL)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<BodhiCRM.Model.VISIT_DETAIL> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<BodhiCRM.Model.VISIT_DETAIL> DataTableToList(DataTable dt)
		{
			List<BodhiCRM.Model.VISIT_DETAIL> modelList = new List<BodhiCRM.Model.VISIT_DETAIL>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				BodhiCRM.Model.VISIT_DETAIL model;
