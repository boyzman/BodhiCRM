...
namespace BodhiCRM.BLL
{
	...
	{
		private readonly BodhiCRM.DAL.FAMILY dal=new BodhiCRM.DAL.FAMILY();
		public FAMILY()
		{}
		#region  BasicMethod
		/// <summary>
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public BodhiCRM.Model.FAMILY GetModelByCache(decimal ID)
		{
			
			string CacheKey = "FAMILYModel-" + ID;
			...
			return (BodhiCRM.Model.FAMILY)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<BodhiCRM.Model.FAMILY> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<BodhiCRM.Model.FAMILY> DataTableToList(DataTable dt)
		{
			List<BodhiCRM.Model.FAMILY> modelList = new List<BodhiCRM.Model.FAMILY>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				BodhiCRM.Model.FAMILY model;
