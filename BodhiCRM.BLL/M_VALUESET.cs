...
namespace BodhiCRM.BLL
{
	...
	{
		private readonly BodhiCRM.DAL.M_VALUESET dal=new BodhiCRM.DAL.M_VALUESET();
		public M_VALUESET()
		{}
		#region  BasicMethod
		/// <summary>
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public BodhiCRM.Model.M_VALUESET GetModelByCache(decimal ID)
		{
			
			string CacheKey = "M_VALUESETModel-" + ID;
			...
			return (BodhiCRM.Model.M_VALUESET)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<BodhiCRM.Model.M_VALUESET> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<BodhiCRM.Model.M_VALUESET> DataTableToList(DataTable dt)
		{
			List<BodhiCRM.Model.M_VALUESET> modelList = new List<BodhiCRM.Model.M_VALUESET>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				BodhiCRM.Model.M_VALUESET model;
